...
class P(object):
    ...
    menu = {
        ...
        'sub' : [
            ['movie', '영화'], ['ktv', '국내TV'],  ['ftv', '외국TV'], ['music', '음악'], ['jav_censored', 'JavCensored'], ['jav_censored_ama', 'JavCensored AMA'], ['jav_uncensored', 'JavUnCensored'], ['book', '책'], ['log', '로그']
        ], 
        'category' : 'tool',
        'sub2' : {
            'ktv' : [
...
def initialize():
    try:
        ...
        P.module_list = [LogicKtv(P), LogicJavCensored(P), LogicJavCensoredAma(P), LogicJavUncensored(P), LogicOttShow(P), LogicMovie(P), LogicFtv(P), LogicLyric(P), LogicBook(P), LogicVideoStation(P), LogicMusic(P)]
        P.logic = Logic(P)
        default_route(P)
