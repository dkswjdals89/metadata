...
# 패키지
from lib_metadata import SiteDaumTv, SiteTmdbTv, SiteTvdbTv, SiteUtil, SiteWatchaTv
...
class LogicFtv(LogicModuleBase):
    ...
    module_map = {'daum':SiteDaumTv, 'tvdb':SiteTvdbTv, 'tmdb':SiteTmdbTv, 'watcha':SiteWatchaTv}
        ...
        if SiteUtil.is_hangul(keyword):
            ...
            if watcha_ret['ret'] == 'success':
                ...
                if en_keyword is None:
                    ...
                    en_keyword = SystemLogicTrans.trans(keyword, source='ko', target='en')
    ...
    def info(self, code):
        try:
            ...
            if daum_search['ret'] == 'success':
                    ...
                    for actor in tvdb_info['actor']:
                        ...
                        for key, value in daum_actor_list.items():
                            ...
                            if actor['is_kor_name']:
                                break
                        #if True and actor['is_kor_name'] == False:
                        #    try:
                        #        actor['name'] = SystemLogicTrans.trans(actor['name'], source='en', target='ko')
                        #        actor['role'] = SystemLogicTrans.trans(actor['role'], source='en', target='ko')
                        #    except:
                        #        pass
                ...
                #for season_no, season in tvdb_info['seasons'].items():
                #    for episode_no, episode in season['episodes'].items():
                #        if episode['is_title_kor'] == False:
                #            episode['title'] = SystemLogicTrans.trans(episode['title'], source='en', target='ko')
                #        if episode['is_plot_kor'] == False:
                #            episode['plot'] = SystemLogicTrans.trans(episode['plot'], source='en', target='ko')
